/* test_put.c - integration test for MKD/CWD/STOR upload flow. */
...
serve_session(int ctrl_listen_fd)
{
    ...
    if (read_line(ctrl_fd, line, sizeof(line)) < 0)
        _exit(10);
    if (strcmp(line, "MKD uploaddir") != 0)
        _exit(11);
    if (write_all(ctrl_fd, "257 Directory created\r\n", 23) < 0)
        _exit(12);

    if (read_line(ctrl_fd, line, sizeof(line)) < 0)
        _exit(13);
    if (strcmp(line, "CWD uploaddir") != 0)
        _exit(14);
    ...
    if (strcmp(received, "hello from put\n") != 0)
        _exit(26);
...
main(void)
{
    ...
    pid_t pid;
    int rc;
    char template[] = "/tmp/ftp_putXXXXXX";
    int tmp_fd;
    const char payload[] = "hello from put\n";
    ...
    rc = ftp_session_mkd(&session, (slice_t){ "uploaddir", 9 }, &reply,
        5000);
    if (rc < 0) {
        fprintf(stderr, "mkd failed: %s\n", strerror(errno));
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
    }

    rc = ftp_session_cwd(&session, (slice_t){ "uploaddir", 9 }, &reply,
        5000);
    if (rc < 0) {
