/* test_put_existing.c - integration test for put flow when MKD fails. */
...
serve_session(int ctrl_listen_fd)
{
    ...
    if (read_line(ctrl_fd, line, sizeof(line)) < 0)
        _exit(10);
    if (strcmp(line, "MKD existingdir") != 0)
        _exit(11);
    if (write_all(ctrl_fd, "550 Access denied\r\n", 19) < 0)
        _exit(12);

    if (read_line(ctrl_fd, line, sizeof(line)) < 0)
        _exit(13);
    if (strcmp(line, "CWD existingdir") != 0)
        _exit(14);
    if (write_all(ctrl_fd, "250 Directory changed\r\n", 23) < 0)
        _exit(15);
    ...
    if (strcmp(received, "hello from existing\n") != 0)
        _exit(26);
...
main(void)
{
    ...
    pid_t pid;
    int rc;
    char template[] = "/tmp/ftp_put_existingXXXXXX";
    int tmp_fd;
    const char payload[] = "hello from existing\n";
    ...
    rc = ftp_session_mkd(&session, (slice_t){ "existingdir", 11 }, &reply,
        5000);
    if (rc == 0) {
        fprintf(stderr, "mkd unexpectedly succeeded\n");
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
    }
    if (errno != EACCES) {
        fprintf(stderr, "wrong errno from mkd: %s\n", strerror(errno));
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
    }

    rc = ftp_session_cwd(&session, (slice_t){ "existingdir", 11 }, &reply,
        5000);
    if (rc < 0) {
