/* test_stor_abort.c - upload cleanup test when the data socket fails. */
...
serve_session(int ctrl_listen_fd)
{
        ...
        _exit(18);
    {
        ...
        setsockopt(data_fd, SOL_SOCKET, SO_LINGER, &ling, sizeof(ling));
    }
    close(data_fd);
    close(data_listen_fd);

    if (write_all(ctrl_fd, "426 Transfer aborted\r\n", 22) < 0)
        _exit(19);
...
main(void)
{
    ...
    pid_t pid;
    int rc;
    char template[] = "/tmp/ftp_stor_abortXXXXXX";
    int tmp_fd;
    const char payload[] = "hello from abort test\n";
    ...
    rc = ftp_session_stor(&session, (slice_t){ "upload.txt", 10 },
        tmp_fd, &reply, 5000);
    if (rc == 0) {
        fprintf(stderr, "stor unexpectedly succeeded\n");
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
    }
    ...
    if (errno == 0) {
        fprintf(stderr, "stor failed without errno\n");
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
    }
    ...
    if (rc < 0) {
        fprintf(stderr, "quit failed after stor abort: %s\n",
            strerror(errno));
        ftp_session_close(&session);
        close(tmp_fd);
        return 1;
