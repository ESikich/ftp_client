/* test_auth_fail.c - integration test for FTP authentication failure. */
...
serve_session(int listen_fd)
{
    ...
    if (read_line(fd, line, sizeof(line)) < 0)
        _exit(7);
    if (strcmp(line, "PASS bad") != 0)
        _exit(8);
    if (write_all(fd, "530 Not logged in\r\n", 19) < 0)
        _exit(9);
...
main(void)
{
    ...
    rc = ftp_session_login(&session, (slice_t){ "demo", 4 },
        (slice_t){ "bad", 3 }, &reply, 5000);
    if (rc == 0) {
        fprintf(stderr, "login unexpectedly succeeded\n");
        ftp_session_close(&session);
        return 1;
    }

    if (errno != EACCES) {
        fprintf(stderr, "wrong errno for auth failure: %s\n",
            strerror(errno));
        ftp_session_close(&session);
        return 1;
    }
